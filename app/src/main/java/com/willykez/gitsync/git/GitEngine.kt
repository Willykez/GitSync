...
object GitEngine {
    ...
    suspend fun cloneRepo(
        ...
        sshTransport: TransportConfigCallback? = null,
    ): GitResult<Git> = io {
        ...
        sshTransport?.let { cmd.setTransportConfigCallback(it) }
        cmd.call()
    }
    ...
    suspend fun commit(
        ...
        amend: Boolean = false,
        sign: Boolean = false, signingKeyId: String? = null,
    ): GitResult<String> = io {
        val id = PersonIdent(name, email)
        val cmd = git.commit().setMessage(message).setAuthor(id).setCommitter(id).setAmend(amend)
        if (sign) {
            GpgSigningSupport.ensureRegistered()
            cmd.setSign(true)
            if (!signingKeyId.isNullOrBlank()) cmd.setSigningKey(signingKeyId)
        } else {
            cmd.setSign(false)
        }
        cmd.call().name()
    ...
    suspend fun fetch(
        ...
        credential: DecryptedCredential? = null, sshTransport: TransportConfigCallback? = null,
    ): GitResult<String> = io {
        val r = git.fetch().setRemote(remote).setRemoveDeletedRefs(prune)
            .also { credential?.let { c -> it.setCredentialsProvider(c.toProvider()) } }
            .also { sshTransport?.let { t -> it.setTransportConfigCallback(t) } }
            .call()
    ...
    suspend fun pullMerge(
        git: Git, credential: DecryptedCredential? = null, sshTransport: TransportConfigCallback? = null,
    ): GitResult<String> = io {
    ...
    suspend fun pullForce(
        git: Git, credential: DecryptedCredential? = null, sshTransport: TransportConfigCallback? = null,
    ): GitResult<String> = io {
        val branch = git.repository.branch
        git.fetch().setRemote("origin")
            .also { credential?.let { c -> it.setCredentialsProvider(c.toProvider()) } }
            .also { sshTransport?.let { t -> it.setTransportConfigCallback(t) } }.call()
    ...
    suspend fun syncMerge(
        ...
        val pull = pullMerge(git, credential, sshTransport)
        if (pull is GitResult.Error) return pull
        val push = push(git, credential = credential, sshTransport = sshTransport)
        if (push is GitResult.Error) return push
        return GitResult.Success("Sync(Merge) done")
    }

    suspend fun syncRebase(
        git: Git, credential: DecryptedCredential? = null, sshTransport: TransportConfigCallback? = null,
    ): GitResult<String> {
        val pull = pullRebase(git, credential, sshTransport)
        if (pull is GitResult.Error) return pull
        val push = push(git, credential = credential, sshTransport = sshTransport)
        if (push is GitResult.Error) return push
    ...
    suspend fun stashSave(git: Git, message: String = "", includeUntracked: Boolean = true): GitResult<String> = io {
        git.stashCreate()
            .setWorkingDirectoryMessage(message.ifBlank { "Git Sync stash" })
    ...
    // ── full-text search ─────────────────────────────────────────────────────
    // Two independent searches: across the working tree's current file contents, and across
    // commit messages in history. Deliberately simple (no index, no persistent cache) — repos
    // this app deals with are small enough on a phone that a linear scan is fine, and it means
    // results are always current instead of possibly-stale like a maintained search index would be.

    private val SEARCH_SKIP_DIRS = setOf(".git")
    private const val SEARCH_MAX_FILE_BYTES = 2 * 1024 * 1024 // matches the file editor's own cap
    private const val SEARCH_MAX_RESULTS = 500

    /** Greps every text file currently in the working tree (not just tracked/committed ones —
     * this is a "what's on disk right now" search, which is what you want while mid-edit). */
    suspend fun searchWorkingTree(
        git: Git, query: String, caseSensitive: Boolean = false,
    ): GitResult<List<SearchMatch>> = io {
        ...
        fun walk(dir: File) {
            ...
            for (f in children) {
                if (results.size >= SEARCH_MAX_RESULTS) return
                if (f.isDirectory) {
                    if (f.name !in SEARCH_SKIP_DIRS) walk(f)
                    continue
                }
                if (f.length() > SEARCH_MAX_FILE_BYTES || f.length() == 0L) continue
                val bytes = try { f.readBytes() } catch (e: Exception) { continue }
                if (RawText.isBinary(bytes)) continue
                val relPath = f.relativeTo(root).path
                val text = String(bytes, Charsets.UTF_8)
                text.lineSequence().forEachIndexed { idx, line ->
                    if (results.size >= SEARCH_MAX_RESULTS) return@forEachIndexed
                    val haystack = if (caseSensitive) line else line.lowercase()
                    val matchStart = haystack.indexOf(needle)
                    if (matchStart >= 0) {
                        results += SearchMatch(relPath, idx + 1, line.trim().take(240), matchStart)
                    }
                }
            }
        }
    ...
    /** Searches commit subject + body text (case-insensitive substring match) across the log. */
    suspend fun searchCommitMessages(
        git: Git, query: String, maxCount: Int = 500,
    ): GitResult<List<CommitInfo>> = io {
        if (query.isBlank()) return@io emptyList()
        val needle = query.lowercase()
        git.log().setMaxCount(maxCount).call()
            .filter { it.fullMessage.lowercase().contains(needle) }
            .map { c ->
                CommitInfo(c.name, c.name.take(7), c.shortMessage, c.fullMessage,
                    c.authorIdent.name, c.authorIdent.emailAddress,
                    c.authorIdent.`when`.time, c.committerIdent.name, c.parentCount,
    ...
    // Stages part of a file's changes instead of the whole file — the standard "git add -p"
    // workflow. Implemented at the plumbing level (compute a line diff between what's
    // currently in the index and what's on disk, then write a new blob containing only the
    // selected lines directly into the index) rather than by shelling out to `git apply
    // --cached`, since JGit's ApplyCommand only ever touches the working tree, not the index.

    /** One contiguous changed region between the index version and the working-tree version
     * of a file, with a little context on each side for readability. [editIndex] identifies
     * this region for [stageHunkSelections] — it's the position of the underlying [Edit] in
     * the diff, not a display-only counter, so it stays valid across hunks appearing/disappearing
     * as long as you re-fetch hunks after every stage (the UI does this automatically). */
    data class DiffHunk(val header: String, val lines: List<DiffHunkLine>, val editIndex: Int)
    data class DiffHunkLine(
        val text: String, val type: HunkLineType,
        val oldLineNo: Int?, val newLineNo: Int?,
        val editIndex: Int, val localIndex: Int, // localIndex: position within this edit's removed/added run; -1 for context
    )
    enum class HunkLineType { CONTEXT, ADDED, REMOVED }

    /** Which lines within each hunk to actually stage. Omitting an editIndex entirely means
     * "stage the whole hunk" (all its removed + added lines) — the common case of tapping
     * a hunk's own "Stage hunk" button rather than hand-picking individual lines. */
    data class HunkSelection(val removedLocalIndices: Set<Int>, val addedLocalIndices: Set<Int>)

    private fun readIndexBlob(repo: Repository, path: String): ByteArray {
        val dirCache = repo.readDirCache()
        val entry = dirCache.getEntry(path) ?: return ByteArray(0)
        repo.newObjectReader().use { reader -> return reader.open(entry.objectId).bytes }
    }

    private fun workingTreeBytes(repo: Repository, path: String): ByteArray {
        val f = File(repo.workTree, path)
        return if (f.exists()) f.readBytes() else ByteArray(0)
    }

    private fun computeEdits(indexBytes: ByteArray, workingBytes: ByteArray): Triple<RawText, RawText, EditList> {
        val a = RawText(indexBytes)
        val b = RawText(workingBytes)
        val edits = EditList()
        edits.addAll(HistogramDiff().diff(RawTextComparator.DEFAULT, a, b))
        return Triple(a, b, edits)
    }

    /** Returns the unstaged hunks for [path] (index version vs working-tree version) so the UI
     * can render per-line checkboxes. Fails with a clear message for binary files, since a line
     * diff isn't meaningful there — whole-file staging is still the right tool for those. */
    suspend fun getWorkingVsIndexHunks(git: Git, path: String, contextLines: Int = 3): GitResult<List<DiffHunk>> = io {
        val repo = git.repository
        val indexBytes = readIndexBlob(repo, path)
        val workingBytes = workingTreeBytes(repo, path)
        if (RawText.isBinary(indexBytes) || RawText.isBinary(workingBytes)) {
            throw IllegalStateException("Binary file — stage the whole file instead")
        }
        val (a, b, edits) = computeEdits(indexBytes, workingBytes)
        edits.mapIndexed { idx, edit ->
            val ctxStart = maxOf(0, edit.beginA - contextLines)
            val ctxEnd = minOf(a.size(), edit.endA + contextLines)
            val lines = mutableListOf<DiffHunkLine>()
            for (i in ctxStart until edit.beginA)
                lines += DiffHunkLine(a.getString(i), HunkLineType.CONTEXT, i + 1, null, idx, -1)
            for (local in 0 until (edit.endA - edit.beginA))
                lines += DiffHunkLine(a.getString(edit.beginA + local), HunkLineType.REMOVED, edit.beginA + local + 1, null, idx, local)
            for (local in 0 until (edit.endB - edit.beginB))
                lines += DiffHunkLine(b.getString(edit.beginB + local), HunkLineType.ADDED, null, edit.beginB + local + 1, idx, local)
            for (i in edit.endA until ctxEnd)
                lines += DiffHunkLine(a.getString(i), HunkLineType.CONTEXT, i + 1, null, idx, -1)
            DiffHunk(
                "@@ -${edit.beginA + 1},${edit.endA - edit.beginA} +${edit.beginB + 1},${edit.endB - edit.beginB} @@",
                lines, idx,
            )
        }
    }

    /** Writes a new index entry for [path] containing the index content with only the
     * selected hunks/lines applied on top — everything else in the working-tree diff for
     * this file stays unstaged, exactly like `git add -p` choosing "yes" on some hunks and
     * "no" on others. Pass an empty [selections] map to stage every hunk in full. */
    suspend fun stageHunkSelections(
        git: Git, path: String, selections: Map<Int, HunkSelection>,
    ): GitResult<Unit> = io {
        val repo = git.repository
        val indexBytes = readIndexBlob(repo, path)
        val workingBytes = workingTreeBytes(repo, path)
        ...
        val out = ByteArrayOutputStream()
        var cursor = 0
        edits.forEachIndexed { idx, edit: Edit ->
            for (i in cursor until edit.beginA) { out.write(a.getString(i).toByteArray(Charsets.UTF_8)); out.write('\n'.code) }
            val sel = selections[idx] // null => stage this hunk in full
            for (local in 0 until (edit.endA - edit.beginA)) {
                val removed = sel == null || local in sel.removedLocalIndices
                if (!removed) { out.write(a.getString(edit.beginA + local).toByteArray(Charsets.UTF_8)); out.write('\n'.code) }
            }
            for (local in 0 until (edit.endB - edit.beginB)) {
                val added = sel == null || local in sel.addedLocalIndices
                if (added) { out.write(b.getString(edit.beginB + local).toByteArray(Charsets.UTF_8)); out.write('\n'.code) }
            }
            cursor = edit.endA
        }
        for (i in cursor until a.size()) { out.write(a.getString(i).toByteArray(Charsets.UTF_8)); out.write('\n'.code) }

        val newContent = out.toByteArray()
        val inserter = repo.newObjectInserter()
        val blobId = inserter.insert(Constants.OBJ_BLOB, newContent)
        inserter.flush()

        val dirCache: DirCache = repo.lockDirCache()
        try {
